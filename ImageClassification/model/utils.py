...
def evaluate_performance_all(network_output, train_samples_gt, train_samples_gt_onehot, m, n, class_count, Test_GT, device, require_AA_KPP=False, printFlag=True):
    ...
    network_output_np = to_numpy(network_output)          # [H*W, C]
    train_samples_gt_np = to_numpy(train_samples_gt)      # [H*W]
    train_samples_gt_onehot_np = to_numpy(train_samples_gt_onehot)  # [H*W, C]
    Test_GT = to_numpy(Test_GT)                           # [m, n]

    # ========== 第二步：计算 OA（用 NumPy）==========
    available_label_idx = (train_samples_gt_np != 0).astype(np.float32)
    available_label_count = available_label_idx.sum()

    pred_labels = np.argmax(network_output_np, axis=1)    # [H*W]
    true_labels = np.argmax(train_samples_gt_onehot_np, axis=1)  # [H*W]

    correct_prediction = np.where(pred_labels == true_labels, available_label_idx, 0.0).sum()
    ...
    # 注意：ground truth 标签从 1 开始，索引从 0 开始
    for i in range(len(train_samples_gt_np)):
        gt_label = int(train_samples_gt_np[i])
        if gt_label != 0:  # 非背景
            cls_idx = gt_label - 1
            count_perclass[cls_idx] += 1
            if pred_labels[i] == cls_idx:  # 预测类别也是 0-based
                correct_perclass[cls_idx] += 1

    # 避免除零
    acc_per_class = np.divide(correct_perclass, count_perclass, out=np.zeros_like(correct_perclass), where=count_perclass!=0)
    test_AA = np.mean(acc_per_class)

    # ========== 第四步：计算 Kappa ==========
    # 将预测结果 reshape 回图像
    pred_map = pred_labels.reshape(m, n)  # 0-based
    test_pre_label_list = []
    ...
    for ii in range(m):
        for jj in range(n):
            if Test_GT[ii, jj] != 0:
                test_pre_label_list.append(pred_map[ii, jj] + 1)  # 转回 1-based
                test_real_label_list.append(Test_GT[ii, jj])       # 本来就是 1-based

    test_pre_label_list = np.array(test_pre_label_list)
    test_real_label_list = np.array(test_real_label_list)

    if len(test_pre_label_list) > 0:
    ...
    # ========== 输出 ==========
    if printFlag:
        print(f"test OA={OA:.4f}, AA={test_AA:.4f}, kpp={kappa:.4f}")
        print('acc per class:')
        print(acc_per_class)
